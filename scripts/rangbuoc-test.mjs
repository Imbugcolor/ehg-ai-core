...
// dọn dữ liệu thử cũ
await sql(`delete from public.booking where ma_booking like 'TEST-%';`);

// ─────────────────────────────────────────────────────────────────────────────
console.log('══ E2 · Booking OTA không bao giờ được đẩy sang PMS ══\n');
{
  await sql(`insert into public.booking (ma_booking, nguon, ten_kenh, ma_xac_nhan_kenh, ten_khach)
             values ('TEST-OTA', 'OTA', 'Agoda', 'AG-TEST-1', 'Khach OTA');`);
  const bId = (await sql(`select id from public.booking where ma_booking = 'TEST-OTA';`))[0].id;

  await sql(`insert into public.booking_segment
             (booking_id, property_id, nguon, ngay_den, ngay_di, loai_phong_coh, so_phong, so_khach_lon)
             values ('${bId}', '${props.BIENXANH}', 'DIRECT', '2026-09-10','2026-09-12','DLX',1,2);`);
  const seg = (await sql(`select id, nguon, sync_enabled from public.booking_segment where booking_id='${bId}';`))[0];

  kt('segment tự lấy nguồn từ booking cha, không tin giá trị truyền vào', seg.nguon === 'OTA',
     `đang là ${seg.nguon}`);
...
{
  await sql(`insert into public.booking (ma_booking, nguon, ten_khach)
             values ('TEST-DIRECT', 'DIRECT', 'Khach Direct');`);
  const bId = (await sql(`select id from public.booking where ma_booking='TEST-DIRECT';`))[0].id;
  await sql(`insert into public.booking_segment
             (booking_id, property_id, nguon, ngay_den, ngay_di, loai_phong_coh, so_phong, so_khach_lon)
             values ('${bId}', '${props.BIENXANH}', 'DIRECT', '2026-09-10','2026-09-12','DLX',1,2);`);
...
console.log('\n══ E4 · Chưa có người duyệt thì không được đẩy ══\n');
{
  const seg = (await sql(`select s.id, s.property_id from public.booking_segment s
                          ...
                          where b.ma_booking='TEST-DIRECT' limit 1;`))[0];

  await sql(`insert into public.sync_job (segment_id, property_id, khoa_chong_trung, payload)
             values ('${seg.id}', '${seg.property_id}', 'TEST-KHOA-1', '{}'::jsonb);`);
  const job = (await sql(`select id from public.sync_job where khoa_chong_trung='TEST-KHOA-1';`))[0];

  const r = await thu(`update public.sync_job set trang_thai='dang_chay' where id='${job.id}';`);
  kt('chạy job chưa duyệt → BỊ CHẶN', !r.ok && /E4/.test(r.msg), r.ok ? 'cho qua!' : r.msg.slice(0, 70));

  await sql(`update public.sync_job set nguoi_duyet='${uid}', duyet_luc=now() where id='${job.id}';`);
  const r2 = await thu(`update public.sync_job set trang_thai='dang_chay' where id='${job.id}';`);
...
console.log('\n══ E10 · Khoá chống trùng ══\n');
{
  const seg = (await sql(`select s.id, s.property_id from public.booking_segment s
                          join public.booking b on b.id=s.booking_id
                          where b.ma_booking='TEST-DIRECT' limit 1;`))[0];
  const r = await thu(`insert into public.sync_job (segment_id, property_id, khoa_chong_trung)
...
{
  await sql(`insert into public.mapping_registry (property_id, loai_ma, ma_coh, ma_pms)
             values ('${props.BIENXANH}', 'room_type', 'DELUXE_TWIN', 'DLX-T')
             on conflict do nothing;`);

  const ok = await sql(`select public.dich_ma('${props.BIENXANH}','room_type','DELUXE_TWIN') as ma;`);
  kt('mã đã map → dịch đúng', ok[0].ma === 'DLX-T', `được ${ok[0].ma}`);

  const r = await thu(`select public.dich_ma('${props.BIENXANH}','room_type','CHUA_MAP');`);
  kt('mã chưa map → NÉM LỖI, không trả giá trị mặc định',
     !r.ok && /MAPPING_THIEU/.test(r.msg), r.ok ? 'trả về giá trị nào đó!' : '');

  const r2 = await thu(`select public.dich_ma('${props.NUIDOI}','room_type','DELUXE_TWIN');`);
...
console.log('\n══ E8 · Ghi vết bất biến ══\n');
{
  const n = await sql(`select count(*) as n from public.audit_log where bang='booking';`);
  ...
  const r2 = await thu(`update public.audit_log set ghi_chu='sua trom' where id=(select min(id) from public.audit_log);`);
